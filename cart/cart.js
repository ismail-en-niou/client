...
function populateCartTable(cartItems, allProducts) {
        ...
        if (product) {
            ...
            tbody.innerHTML += `
                <tr id="row-${productId}">
                    ...
                    <td><img  loading="lazy" src="https://electro-api.robixe.online/image/${product.img}" alt=""></td>
                    <td>${product.name}</td>
    ...
    document.getElementById("subtotal").innerHTML = `
        ...
        <button class="normal" id="btn">Proceed to checkout</button>
    `;
        ...
        async function updateCartOnServer() {
            try {
                ...
                const response = await fetch('https://electro-api.robixe.online/cart', {
                    ...
                    body: JSON.stringify({ token: token, cart: cart })
                });
...
if (!token) {
    ...
    fetch('https://electro-api.robixe.online/info', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    .then(data => {
        ...
        if (cartItems) {
            // Convert cart data from the API to an array
            let sells = Object.keys(cartItems).map(key => ({
                productId: key,
                productQuantity: cartItems[key]
            }));

            localStorage.setItem("sells", JSON.stringify(sells));
            handleCartData(sells);
        } else {
            const cartItems = JSON.parse(localStorage.getItem('sells')) || [];
            handleCartData(cartItems);
        }
...
document.addEventListener('change', async (event) => {
    if (event.target.matches('input[type="number"]') && event.target.value < 100 && event.target.value > 0) {
        ...
        if (token) {
            ...
            // Create the cart object to send
            const cart = sells.reduce((acc, item) => {
                acc[item.productId] = Number(item.productQuantity);
                return acc;
            }, {});
